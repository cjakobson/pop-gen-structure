function [] = plot_neighbor_rare_common_af(dependency_directory)
...
for i=1:length(af_thresh)
    ...
    for j=1:length(aa_labels)
        ...
        for k=1:length(structure_labels)
            ...
            v1=reshape(neighbor_mat_1K(logical(temp_idx1.*temp_idx2.*temp_idx3)),1,[]);
            v2=reshape(neighbor_mat_1K(logical(~temp_idx1.*temp_idx2.*temp_idx3)),1,[]);
