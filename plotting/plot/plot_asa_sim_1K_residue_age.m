function [] = plot_asa_sim_1K_residue_age(dependency_directory)
...
gene_residue_asa_mat=nan(length(genes_to_use),length(aa_labels));
for i=1:length(aa_labels)
    ...
    for j=1:length(genes_to_use)
        ...
        gene_residue_asa_mat(j,i)=mean(temp_asa_1K,'omitnan')/...
...
for i=1:length(aa_labels)
    ...
    v1=gene_residue_asa_mat(old_idx,i);
    v2=gene_residue_asa_mat(young_idx,i);
