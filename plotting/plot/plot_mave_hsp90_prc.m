function [] = plot_mave_hsp90_prc(dependency_directory,plot_offset)
...
for k=1:length(v_fit_thresh)
    ...
    for i=1:length(v_bins_asa)
        ...
        for j=1:length(v_bins_neighbors)
            ...
            precision(m)=sum(predicted_unfit_idx.*actual_unfit_idx)/...
                sum(predicted_unfit_idx);  %precision
            recall(m)=sum(predicted_unfit_idx.*actual_unfit_idx)/...
                sum(actual_unfit_idx); %recall
            m=m+1;
    ...
    hold on
    scatter(recall,precision,10*k,'k','filled')
    %plot(fpr,tpr,'-k')
    axis square
    xlim([0 1])
    ylim(xlim)
